...
with st.expander("Filteroptionen"):
    ...
    if st.session_state["city_filter"] != "Alle":
        temp_filtered = temp_filtered[
            temp_filtered["patientCity"] == st.session_state["city_filter"]
        ]
    ...
    if st.session_state["street_filter"] != "Alle":
        temp_filtered = temp_filtered[
            temp_filtered["patientStreet"] == st.session_state["street_filter"]
        ]

    # House number filter with "Alle" option, dynamic based on city and street
    house_options = ["Alle"] + list(
        temp_filtered["patientHouseNumber"].dropna().unique()
    )
...
if st.session_state["city_filter"] != "Alle":
    filtered_df = filtered_df[
        filtered_df["patientCity"] == st.session_state["city_filter"]
    ]

if st.session_state["street_filter"] != "Alle":
    filtered_df = filtered_df[
        filtered_df["patientStreet"] == st.session_state["street_filter"]
    ]

if st.session_state["house_number_filter"] != "Alle":
    filtered_df = filtered_df[
        filtered_df["patientHouseNumber"] == st.session_state["house_number_filter"]
    ]
...
mask = (filtered_df["alarmTime"] >= start_date_utc) & (
    filtered_df["alarmTime"] <= end_date_utc
)
...
if "staticMissionType" in filtered_df.columns:
    ...
    if krankentransport_values:
        filtered_df = filtered_df[
            filtered_df["staticMissionType"].isin(krankentransport_values)
        ]
        st.write(f"**Gefiltert nach: {krankentransport_values}**")
    else:
        # Fallback to exact match if no values found
        filtered_df = filtered_df[
            filtered_df["staticMissionType"] == "Krankentransport"
        ]
...
if "emergencyCareType" in filtered_df.columns:
    filtered_df["week"] = (
        filtered_df["missionDate"].dt.to_period("W").apply(lambda r: r.start_time)
    )
    weekly_counts = (
        filtered_df.groupby(["week", "emergencyCareType"])
        .size()
...
if "missionType" in filtered_df.columns:
    ...
    weekly_counts = (
        filtered_df.groupby(["week", "missionType"]).size().reset_index(name="counts")
    )
...
st.write(
    "Aufgrund von mehrfach allamierungen existieren zu einzelnen NIDA-Protokollen mehrere ETÜ-Datensätze."
)
...
merged_df = pd.merge(
    filtered_df, etu, left_on="missionNumber", right_on="EINSATZ_NR", how="left"
)
st.write(merged_df)
...
if not sankey_data.empty:
    ...
    st.warning(
        "Keine Daten mit sowohl ETU-Diagnose als auch endgültiger Diagnose verfügbar für das Sankey-Diagramm."
    )
...
if "missionType" in filtered_df.columns:
    ...
    ktw_protocols = filtered_df[
        filtered_df["missionType"].str.contains(
            r"\bktw\b", case=False, na=False, regex=True
        )
        & ~filtered_df["missionType"].str.contains("s-ktw", case=False, na=False)
        & ~filtered_df["missionType"].str.contains(
            r"\brtw\b", case=False, na=False, regex=True
        )
    ]["protocolId"]
    ...
    st.write(
        f"**KTW-Anamnesis-Daten: {len(ktw_anamnesis_df)} Protokolle für KTW-Transporte gefunden**"
    )
else:
    ktw_anamnesis_df = anamnesis_df.copy()  # Fallback to all anamnesis data
    st.warning(
        "Spalte 'missionType' nicht gefunden - KTW-Filter für Anamnese wird übersprungen."
    )
...
if not ktw_anamnesis_df.empty:
    ...
    text_column = (
        "content"
        if "content" in ktw_anamnesis_df.columns
        else ("text" if "text" in ktw_anamnesis_df.columns else None)
    )
    ...
    ktw_anamnesis_df["medical_care"] = enhanced_requirements.apply(
        lambda x: x["medical_care"]
    )
    ktw_anamnesis_df["ktw_equipment"] = enhanced_requirements.apply(
        lambda x: x["ktw_equipment"]
    )
    ktw_anamnesis_df["infectious_disease"] = enhanced_requirements.apply(
        lambda x: x["infectious_disease"]
    )
    ktw_anamnesis_df["crew_assessment"] = enhanced_requirements.apply(
        lambda x: x["crew_assessment"]
    )
    ktw_anamnesis_df["krankenfahrt_mentioned"] = enhanced_requirements.apply(
        lambda x: x["krankenfahrt_mentioned"]
    )
    ...
    st.metric(
        "Anzahl ohne Dokumentation",
        f"{len(none_all_three)} ({(len(none_all_three)/len(ktw_anamnesis_df)*100):.1f}%)",
    )
    ...
    fig_medical = px.pie(
        values=medical_counts.values,
        names=medical_labels,
        title="Medizinische Betreuung notwendig (KTW)",
        color_discrete_sequence=[
            chart_colors.get(x, "#95a5a6") for x in medical_counts.index
        ],
    )
    ...
    fig_equipment = px.pie(
        ...
        color_discrete_sequence=[
            chart_colors.get(x, "#95a5a6") for x in equipment_counts.index
        ],
    )
    ...
    fig_infection = px.pie(
        values=infection_counts.values,
        names=infection_labels,
        title="Hinweise auf eine schwere ansteckende Krankheit (KTW)",
        color_discrete_sequence=[
            chart_colors.get(x, "#95a5a6") for x in infection_counts.index
        ],
    )
    ...
    if crew_counts.sum() > 0:
        ...
        fig_crew = px.pie(
            values=crew_counts.values,
            names=crew_labels,
            title="Einschätzung der Besatzung ob Krankentransport indiziert (KTW)",
            color_discrete_sequence=[
                chart_colors.get(x, "#95a5a6") for x in crew_counts.index
            ],
        )
    ...
    doc_rates = [
        (
            (
                len(ktw_anamnesis_df)
                - medical_counts.loc[pd.isna(medical_counts.index)].sum()
                if pd.isna(medical_counts.index).any()
                else 0
            )
            / len(ktw_anamnesis_df)
            * 100
        ...
        (
            (
                len(ktw_anamnesis_df)
                - equipment_counts.loc[pd.isna(equipment_counts.index)].sum()
                if pd.isna(equipment_counts.index).any()
                else 0
            )
            / len(ktw_anamnesis_df)
            * 100
        ...
        (
            (
                len(ktw_anamnesis_df)
                - infection_counts.loc[pd.isna(infection_counts.index)].sum()
                if pd.isna(infection_counts.index).any()
                else 0
            )
            / len(ktw_anamnesis_df)
            * 100
    ...
    fig_doc.update_traces(
        text=[f"{rate:.1f}%" for rate in doc_rates], textposition="outside"
    )
    ...
    ktw_anamnesis_df["categories_needed"] = ktw_anamnesis_df.apply(
        count_needed_categories, axis=1
    )
    categories_needed_dist = (
        ktw_anamnesis_df["categories_needed"].value_counts().sort_index()
    )

    fig_categories = px.bar(
        x=categories_needed_dist.index,
        y=categories_needed_dist.values,
    ...
    fig_categories.update_traces(
        text=categories_needed_dist.values, textposition="outside"
    )
