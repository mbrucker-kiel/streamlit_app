...
bewusstsein_gemindert = gcs_df[mask_gcs_lt_15 & mask_gcs_gt_0 & mask_type_neuro]
...
prozent_gesamt = (
    (anzahl_mit_bz / anzahl_bewusstseinsgemindert * 100)
    if anzahl_bewusstseinsgemindert > 0
    else 0
)
...
merged_df = merged_df.rename(columns={"value": "bz_value"})
merged_df_sorted = merged_df.sort_values(by=["bz_value"], ascending=True)

st.write(merged_df_sorted)
