...
if 'missionDate' in merged_df.columns:
    ...
    filtered_df = merged_df[
        (mission_dates >= start_dt) & 
        (mission_dates <= end_dt)
    ].copy()
else:
...
if 'callSign' in filtered_df.columns:
    ...
    selected_callsigns = st.multiselect(
        ...
        default=[v.strip() for v in DEFAULT_NIDA_VEHICLES if v.strip() in available_callsigns]
    )
...
st.write("Berechnung der Auslastung basierend auf StatusAlarm und StatusEnd der NIDA-Protokolle.")
...
if 'duration_hours' in filtered_df.columns:
    ...
    longest_missions = filtered_df[['protocolId', 'callSign', 'StatusAlarm', 'StatusEnd', 'duration_hours', 'statisticMissionType']].copy()
...
if 'duration_hours' in filtered_df.columns:
    ...
    top_10_longest = filtered_df.nlargest(10, 'duration_hours')[
        ['protocolId', 'callSign', 'StatusAlarm', 'StatusEnd', 'duration_hours', 'statisticMissionType']
    ].copy()
    ...
    st.dataframe(top_10_longest[['protocolId', 'callSign', 'StatusAlarm', 'StatusEnd', 'duration_hours', 'duration_minutes', 'statisticMissionType']])
    ...
    if not valid_durations.empty:
        ...
        st.write(f"- Durchschnittliche Dauer: {valid_durations.mean():.2f} Stunden ({valid_durations.mean()*60:.1f} Minuten)")
        st.write(f"- Median Dauer: {valid_durations.median():.2f} Stunden ({valid_durations.median()*60:.1f} Minuten)")
        st.write(f"- Maximale Dauer: {valid_durations.max():.2f} Stunden ({valid_durations.max()*60:.1f} Minuten)")
        st.write(f"- Minimale Dauer: {valid_durations.min():.2f} Stunden ({valid_durations.min()*60:.1f} Minuten)")
        
        # Flag suspicious durations
        suspicious = filtered_df[filtered_df['duration_hours'] > 24]  # Missions longer than 24 hours
        if not suspicious.empty:
            st.warning(f"⚠️ {len(suspicious)} Einsätze dauern länger als 24 Stunden - mögliche Datenfehler!")
...
if 'StatusAlarm' in filtered_df.columns and 'StatusEnd' in filtered_df.columns:
    ...
    filtered_df['StatusAlarm'] = pd.to_datetime(filtered_df['StatusAlarm'], errors='coerce')
    filtered_df['StatusEnd'] = pd.to_datetime(filtered_df['StatusEnd'], errors='coerce')
    
    # Calculate duration in hours
    filtered_df['duration_hours'] = (filtered_df['StatusEnd'] - filtered_df['StatusAlarm']).dt.total_seconds() / 3600
    ...
    if 'StatusAlarm' in filtered_df.columns:
        ...
        daily_util = filtered_df.groupby(['callSign', 'weekday'])['duration_hours'].sum().reset_index()
        ...
        grouped_daily = daily_util.groupby(['callSign', 'group'])['duration_hours'].sum().reset_index()
        ...
        def get_available_hours(group, callsign):
            ...
            if group == 'Mon-Thu':
                return daily_hours * (weekday_counts.get('Monday', 0) + weekday_counts.get('Tuesday', 0) + 
                                    weekday_counts.get('Wednesday', 0) + weekday_counts.get('Thursday', 0))
            elif group == 'Fri':
        ...
        for callsign in selected_callsigns:
            for group in ['Mon-Thu', 'Fri', 'Sat', 'Sun']:
                ...
                actual_row = grouped_daily[(grouped_daily['callSign'] == callsign) & (grouped_daily['group'] == group)]
                actual = actual_row['duration_hours'].sum() if not actual_row.empty else 0
        ...
        if not daily_df.empty:
            try:
                fig_daily = px.bar(daily_df, x='group', y='percentage', color='callSign', 
                                  title='Tägliche Auslastung in Prozent', barmode='group')
...
if 'statisticMissionType' in filtered_df.columns:
    mission_type_counts = filtered_df['statisticMissionType'].value_counts().reset_index()
    ...
    if not mission_type_counts.empty:
        fig_mission = px.bar(mission_type_counts, x='Mission Type', y='Count', 
...
if 'callSign' in merged_df.columns and 'missionType' in merged_df.columns:
    ...
    if not sktw_missions.empty:
        ...
        sktw_missions['vehicle_type'] = sktw_missions['callSign'].apply(classify_vehicle_type)
        ...
        def classify_mission_type(mission_type):
            ...
            elif 's-ktw - kein transport' in mission_str or 'kein-transport' in mission_str:
                return 'S-KTW – Kein Transport'
            else:
                return 'Andere S-KTW'

        sktw_missions['mission_category'] = sktw_missions['missionType'].apply(classify_mission_type)

        # Create summary table
        summary_df = sktw_missions.groupby(['vehicle_type', 'mission_category']).size().reset_index(name='count')
        ...
        # Add totals
        pivot_table['Gesamt'] = pivot_table.select_dtypes(include=[int, float]).sum(axis=1)
        ...
        if not summary_df.empty:
            fig_sktw = px.bar(
                ...
                barmode='group',
                labels={'vehicle_type': 'Fahrzeugtyp', 'count': 'Anzahl Missionen', 'mission_category': 'Mission-Kategorie'}
            )
