...
if 'leadingDiagnosis' not in df_index.columns:
    ...
    mask_stemi = (
        df_index['leadingDiagnosis'].str.lower().str.contains('stemi', na=False) &
    ...
    if anzahl_stemi == 0:
        st.warning("Keine STEMI-Fälle gefunden! Überprüfen Sie die Daten oder die Filterkriterien.")
    else:
        ...
        prozent_gesamt = (anzahl_mit_ass / anzahl_stemi * 100) if anzahl_stemi > 0 else 0
        ...
        stemi_mit_ass_status = pd.DataFrame({
            ...
            'ASS_gegeben': [pid in ass_gabe['protocolId'].unique() for pid in stemi_protokoll_ids]
        })
        ...
        status_counts['ASS gegeben'] = status_counts['ASS gegeben'].map({True: 'Ja', False: 'Nein'})
        ...
        detail_view_sorted = detail_view.sort_values(by=['ASS_gegeben'], ascending=False)
        ...
        if anzahl_mit_ass > 0:
            ...
            ass_details = ass_gabe[['protocolId', 'med_name', 'dose', 'dose_unit', 'route']]
