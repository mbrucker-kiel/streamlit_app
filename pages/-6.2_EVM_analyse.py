...
st.write("Diese Seite analysiert erweiterte Versorgungsmaßnahmen (EVM) in Notfalleinsätzen.")
...
if "missionDate" in merged_df.columns:
    ...
    if "missionType" in evm_time_df.columns:
        available_ts_mission_types = sorted(evm_time_df["missionType"].dropna().unique())
        ...
        if selected_ts_mission_types:
            evm_time_df = evm_time_df[evm_time_df["missionType"].isin(selected_ts_mission_types)].copy()
    ...
    monthly_evm = evm_time_df.groupby(["year_month", "vehicleType"]).\
        size().reset_index(name="evm_count")
    monthly_total = total_missions_filtered.assign(
        year_month=total_missions_filtered["missionDate"].dt.to_period("M").astype(str)
    ).groupby(["year_month", "vehicleType"])["protocolId"].count().\
...
if "missionDate" in index_df.columns and "missionType" in index_df.columns and "evmCount" in index_df.columns:
    ...
    if not index_filtered.empty:
        ...
        mission_evm_stats = chart_data.groupby("missionType").agg({
            "evmCount": ["count", lambda x: (x > 0).sum()],  # Total missions, missions with EVM
        }).reset_index()
        ...
        mission_evm_stats["evm_percentage"] = (
            mission_evm_stats["evm_missions"] / mission_evm_stats["total_missions"] * 100
        ).round(2)

        # Sort by EVM percentage descending
        mission_evm_stats = mission_evm_stats.sort_values("evm_percentage", ascending=False)
    ...
    st.warning("Erforderliche Spalten (missionDate, missionType, evmCount) nicht in index_df gefunden.")
...
if "missionDate" in index_df.columns and "missionType" in index_df.columns and "evmCount" in index_df.columns:
    ...
    if not index_filtered.empty:
        ...
        kein_transport_missions = index_filtered[
            index_filtered["missionType"].str.contains("kein.?transport", case=False, na=False, regex=True)
        ].copy()

        if not kein_transport_missions.empty:
            st.write(f"**'Kein Transport' Missionen im Zeitraum:** {len(kein_transport_missions)}")

            # Get EVM data for these missions
            evm_kein_transport = evm_df[evm_df["protocolId"].isin(kein_transport_missions["protocolId"])].copy()
            ...
            if not evm_kein_transport.empty:
                ...
                vehicle_evm_stats = evm_kein_transport_with_vehicle.groupby("vehicleType").agg({
                    "protocolId": "count",  # Total EVM entries per vehicle type
                }).reset_index()

                vehicle_evm_stats = vehicle_evm_stats.rename(columns={"protocolId": "evm_entries"})

                # Get total "kein transport" missions per vehicle type
                total_kein_transport_by_vehicle = kein_transport_missions.merge(
                    merged_df[["protocolId", "vehicleType"]],
                    on="protocolId",
                    how="left"
                ).groupby("vehicleType").size().reset_index(name="total_missions")
                ...
                vehicle_evm_stats["evm_percentage"] = (
                    vehicle_evm_stats["evm_entries"] / vehicle_evm_stats["total_missions"] * 100
                ).round(2)

                # Display results
                st.write("**EVM-Nutzung bei 'Kein Transport' Missionen nach Fahrzeugtyp:**")
                st.dataframe(vehicle_evm_stats)

                # Calculate overall percentage
                total_evm_entries = vehicle_evm_stats["evm_entries"].sum()
                total_kein_transport_missions = vehicle_evm_stats["total_missions"].sum()
                overall_percentage = (total_evm_entries / total_kein_transport_missions * 100) if total_kein_transport_missions > 0 else 0

                st.write(f"**Gesamt EVM-Rate bei 'Kein Transport' Missionen:** {overall_percentage:.2f}%")
                st.write(f"({total_evm_entries} von {total_kein_transport_missions} Missionen hatten EVM-Maßnahmen)")
                ...
                if not evm_kein_transport_with_vehicle.empty:
                    ...
                    evm_descriptions = evm_kein_transport_with_vehicle["description"].value_counts().reset_index()
                    ...
                    st.write("**Detailansicht: Missionen mit EVM-Maßnahmen (kein Transport):**")
                    st.write(
                    ...
                    evm_summary = evm_kein_transport.groupby('protocolId').agg({
                        'description': lambda x: ', '.join(sorted(set(x.dropna()))),
                        'type': lambda x: ', '.join(sorted(set(x.dropna())))
                    }).reset_index()
                    evm_summary = evm_summary.rename(columns={
                    ...
                    detailed_evm_missions = evm_kein_transport_with_vehicle.merge(
                        merged_df[
                            ["protocolId", "missionType", "missionDate", "callSign", 
                    ...
                    display_columns = [
                        "protocolId", "missionDate", "callSign", "vehicleType",
                        "missionType", "content_destinationFacility", "all_evm_measures", 
                        "evm_categories", "description", "applicant", "timestamp"
                    ]
                    available_columns = [
                    ...
                    st.write("**Zusammenfassung der EVM-Maßnahmen bei 'Kein Transport' Missionen:**")
                    
                    # Group by description and vehicle type
                    evm_by_type_vehicle = detailed_evm_missions.groupby(
            ...
            st.info("Keine 'Kein Transport' Missionen im ausgewählten Zeitraum gefunden.")
    else:
        st.warning("Keine Daten im ausgewählten Zeitraum gefunden.")
else:
...
if "callSign" in merged_df.columns:
    ...
    available_vehicles = sorted([
        cs for cs in merged_df["callSign"].dropna().unique()
...
if selected_vehicles and "missionDate" in merged_df.columns:
    ...
    if not comparison_df.empty:
        ...
        total_missions = comparison_df.groupby("callSign").size().\
            reset_index(name="total_missions")
        ...
        missions_with_evm = comparison_df[
            comparison_df["protocolId"].isin(evm_protocols)
        ].groupby("callSign").size().reset_index(name="missions_with_evm")
        ...
        comparison_stats["evm_percentage"] = (
            ...
            comparison_stats["total_missions"] * 100
        ).round(2)

        # Add vehicle type classification
        comparison_stats["vehicleType"] = comparison_stats["callSign"].\
        ...
        comparison_df["year_month"] = comparison_df["missionDate"].\
        ...
        for vehicle in selected_vehicles:
            ...
            if not vehicle_data.empty:
                monthly_vehicle = vehicle_data.groupby("year_month").agg({
                    "protocolId": "count"  # Total missions per month
                }).reset_index()
                ...
                evm_monthly = vehicle_data[
                    vehicle_data["protocolId"].isin(evm_protocols)
                ].groupby("year_month").size().reset_index(name="missions_with_evm")
                ...
                monthly_vehicle["evm_percentage"] = (
                    ...
                    monthly_vehicle["total_missions"] * 100
                ).round(2)
        ...
        if "missionType" in comparison_df.columns:
            ...
            total_by_type = comparison_df.groupby("missionType").size().\
                reset_index(name="total_missions")

            # Missions with EVM by mission type
            evm_by_type = comparison_df[
                comparison_df["protocolId"].isin(evm_protocols)
            ].groupby("missionType").size().reset_index(name="missions_with_evm")
            ...
            mission_comparison["evm_percentage"] = (
                ...
                mission_comparison["total_missions"] * 100
            ).round(2)
        ...
        with col2:
            st.metric(
                "Höchste EVM %",
                f"{comparison_stats['evm_percentage'].max():.1f}%"
            )

        with col3:
            st.metric(
                "Gesamtmissionen",
                f"{comparison_stats['total_missions'].sum()}"
            )
