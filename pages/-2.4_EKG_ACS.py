...
df_index = df_index[
    df_index["missionType"].isin(["Pauschale RTW", "RTW - Transport", "Pauschale NEF"])
]
...
if "leadingDiagnosis" not in df_index.columns:
    ...
    mask_acs = (
        ...
        | df_index["leadingDiagnosis"]
        .str.lower()
        .str.contains("brustschmerz", na=False)
        | df_index["leadingDiagnosis"]
        .str.lower()
        .str.contains("thoraxschmerz", na=False)
    ...
    if anzahl_acs == 0:
        st.warning(
            "Keine ACS-Fälle gefunden! Überprüfen Sie die Daten oder die Filterkriterien."
        )
    else:
        ...
        acs_mit_ekg_status = pd.DataFrame(
            {
                ...
                "EKG_durchgeführt": [
                    pid in ekg_acs["protocolId"].unique() for pid in acs_protokoll_ids
                ],
            }
        )
        ...
        status_counts = (
            acs_mit_ekg_status["EKG_durchgeführt"].value_counts().reset_index()
        )
        status_counts.columns = ["EKG durchgeführt", "Anzahl"]
        status_counts["EKG durchgeführt"] = status_counts["EKG durchgeführt"].map(
            {True: "Ja", False: "Nein"}
        )
        ...
        detail_view_sorted = detail_view.sort_values(
            by=["EKG_durchgeführt"], ascending=True
        )
